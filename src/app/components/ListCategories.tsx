...
import {Input, List, Row, Popconfirm, message, Button} from "antd";
    ...
    const list = this.state.isExtended ? (
      <List dataSource={listData} renderItem={(item: { title: string, id: number }) => (
        item.id != this.state.editingId ? <List.Item key={item.title} actions={[
            ...
            <Popconfirm title="Category will be deleted!" onConfirm={() => this.deleteConfirmHandler(item.id)}
                        onCancel={() => this.deleteCancelHandler} okText="Agree" cancelText="Cancel">
          ...
          <Form>
            <List.Item key={item.title}
                       actions={[<div onClick={() => this.saveCategoryHandler(item.id, item.title)}><Icon
                         type={'save'}/></div>,
                         <div onClick={this.closeEditHandler}><Icon type={'rollback'}/></div>]}>
              <Form.Item>
